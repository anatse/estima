...
func (ps *ProcessService) ConfigRoutes (router *mux.Router, handler HandlerOfHandlerFunc) {
	...
	router.Handle ("/stage/{id}/process/create", handler(http.HandlerFunc(ps.create))).Methods("POST").Name("Create process within projects stage")
	router.Handle ("/process/{id}/status", handler(http.HandlerFunc(ps.setStatus))).Methods("POST").Name("Create process within projects stage")
	router.Handle ("/process/{id}/remove", handler(http.HandlerFunc(ps.remove))).Methods("POST", "DELETE").Name("Create process within projects stage")
	router.Handle ("/process/{id}/feature/list", handler(http.HandlerFunc(ps.listFeatures))).Methods("POST").Name("Create process within projects stage")
	router.Handle ("/process/{id}", handler(http.HandlerFunc(ps.findOne))).Methods("POST", "GET").Name("Create process within projects stage")
